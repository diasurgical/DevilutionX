...
void SoftwareRenderer::EndViewportZoom(int viewportHeight)
{
	...
	Surface fullOut(palSurface_, MakeSdlRect(0, 0, gnScreenWidth, gnScreenHeight));
...
void SoftwareRenderer::SetClipRegion(Rectangle region)
{
	clipRect_ = MakeSdlRect(region);
	hasClip_ = true;
}
...
void SoftwareRenderer::DrawBlackTile(int sx, int sy)
{
	...
	Surface out(palSurface_, MakeSdlRect(0, 0, gnScreenWidth, viewportH));
...
void SoftwareRenderer::FillRect(Rectangle rect, uint8_t colorIndex)
{
	Surface out = MakeTargetSurface(palSurface_, hasClip_, clipRect_);
	const Point rel = ToRelative(rect.position, hasClip_, clipRect_);
	devilution::FillRect(out, rel.x, rel.y, rect.size.width, rect.size.height, colorIndex);
}

void SoftwareRenderer::DrawBlendedRect(Rectangle rect)
{
	Surface out = MakeTargetSurface(palSurface_, hasClip_, clipRect_);
	const Point rel = ToRelative(rect.position, hasClip_, clipRect_);
	devilution::DrawBlendedRectTo(out, rel.x, rel.y, rect.size.width, rect.size.height);
}
...
void SoftwareRenderer::ApplyTRN(Rectangle rect, const uint8_t *trn, uint8_t skipColorIndicesBelow)
{
	Surface out = MakeTargetSurface(palSurface_, hasClip_, clipRect_);
	const Point rel = ToRelative(rect.position, hasClip_, clipRect_);
	int x = rel.x;
...
void SoftwareRenderer::TintRect(Rectangle rect, uint8_t colorIndex)
{
