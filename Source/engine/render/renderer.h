...
public:
	...
	// --- Primitives ---
	virtual void FillRect(Rectangle rect, uint8_t colorIndex) = 0;
	virtual void DrawBlendedRect(Rectangle rect) = 0;
	 ...
	 * @param rect Absolute screen region.
	 * @param colorIndex The base palette index for the slot color (e.g. PAL16_BLUE+1).
	 */
	virtual void TintRect(Rectangle rect, uint8_t colorIndex) = 0;
	 ...
	 */
	virtual void ApplyTRN(Rectangle rect, const uint8_t *trn, uint8_t skipColorIndicesBelow = 0) = 0;
	...
	// --- Clipping ---
	virtual void SetClipRegion(Rectangle region) = 0;
...
/**
 * @brief The active renderer. Use GetRenderer() instead of reading this directly.
 *
 * Non-owning; ownership lives in renderer.cpp. Exposed only so that
 * GetRenderer() can be inlined at its ~365 call sites.
 */
extern DVL_API_FOR_TEST Renderer *CurrentRenderer;

/**
 * @brief Returns the active renderer instance.
 *
 * Cheap enough to call repeatedly (a single load), but note that the compiler
 * cannot hoist it out of a loop containing renderer calls. In hot draw loops,
 * bind a local `Renderer &renderer = GetRenderer();` once instead.
 */
