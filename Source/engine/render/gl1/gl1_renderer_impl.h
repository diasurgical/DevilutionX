...
public:
	...
	{
		Gl1FillRect(Rectangle { { 0, 0 }, { gnScreenWidth, gnScreenHeight } }, 0);
	}
...
protected:
	    ...
	    bool transparency, TileProperties tileProperties, bool isFloor) override
	{
		...
		if (const LevelCelBlock levelCelBlock { pMap->mt[1] }; levelCelBlock.hasValue()) {
			...
			if (!isFloor || tileType == TileType::TransparentSquare) {
				if (isFloor && tileType == TileType::TransparentSquare) {
					Gl1DrawTile(position + Displacement { DunFrameWidth, -16 }, TileType::TransparentSquare,
					    GetDunFrameFoliage(pDungeonCels.get(), levelCelBlock.frame()),
					    16, MaskType::Solid, tbl, lightingMode_);
				} else {
	...
	void FillRect(Rectangle rect, uint8_t colorIndex) override
	{
		Gl1FillRect(rect, colorIndex);
	}

	void DrawBlendedRect(Rectangle rect) override
	{
		Gl1DrawBlendedRect(rect);
	}
	...
	void TintRect(Rectangle rect, uint8_t colorIndex) override
	{
		Gl1TintRect(rect, colorIndex);
	}

	void ApplyTRN(Rectangle /*rect*/, const uint8_t * /*trn*/, uint8_t /*skipColorIndicesBelow*/) override
	{
	...
	void SetClipRegion(Rectangle region) override
	{
		Gl1SetClipRegion(region);
	}
