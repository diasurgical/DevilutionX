...
void Gl1SetClipRegion(Rectangle region)
{
	EnsureFrameBegun();
	ApplyScissor(region.position.x, region.position.y, region.size.width, region.size.height);
}
...
void Gl1FillRect(Rectangle rect, uint8_t colorIndex)
{
	const auto [x, y] = rect.position;
	const auto [width, height] = rect.size;
	EnsureFrameBegun();
...
void Gl1DrawBlendedRect(Rectangle rect)
{
	const auto [x, y] = rect.position;
	const auto [width, height] = rect.size;
	EnsureFrameBegun();
...
void Gl1TintRect(Rectangle rect, uint8_t colorIndex)
{
	const auto [x, y] = rect.position;
