 ...
 */
void DrawFloorTile(Renderer &renderer, Point tilePosition, Point targetBufferPosition)
{
	const int lightTableIndex = dLight[tilePosition.x][tilePosition.y];
	const uint16_t levelPieceId = dPiece[tilePosition.x][tilePosition.y];
	renderer.DrawFloorLevelTile(targetBufferPosition, levelPieceId, lightTableIndex);
}
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < columns; j++, tilePosition += Direction::East, targetBufferPosition.x += TILE_WIDTH) {
			...
			if (IsFloor(tilePosition)) {
				DrawFloorTile(renderer, tilePosition, targetBufferPosition);
			}
		}
...
void DrawOOB(Point tilePosition, Point targetBufferPosition, int rows, int columns)
{
	...
	for (int i = 0; i < rows + 5; i++) { // 5 extra rows needed to make sure everything gets rendered at the bottom half of the screen
		for (int j = 0; j < columns; j++, tilePosition += Direction::East, targetBufferPosition.x += TILE_WIDTH) {
			if (!InDungeonBounds(tilePosition)) {
				if (leveltype == DTYPE_TOWN) {
					renderer.DrawBlackTile(targetBufferPosition.x, targetBufferPosition.y);
				} else {
...
void DrawGame(Point position, Displacement offset)
{
	...
	Renderer &renderer = GetRenderer();
	renderer.BeginScene(position, Point {} + offset, rows, columns);
	renderer.BeginViewportZoom();
	...
	renderer.SetClipRegion({ { 0, 0 }, { gnScreenWidth, clipHeight } });
	...
	renderer.EndViewportZoom(gnViewportHeight);
	renderer.EndScene();
...
/**
 * @brief Which parts of the main panel a frame needs to redraw.
 *
 * Components the panel retains across frames can be skipped, but renderers that clear the
 * whole frame each present must redraw all of them.
 */
struct PanelDrawFlags {
	bool mainPanel;
	bool health;
	bool mana;
	bool controlButtons;
...
/**
 * @brief Draws the dungeon view and the HUD on top of it, without presenting.
 */
void DrawScene(const PanelDrawFlags &draw)
{
	...
	if (draw.health) {
		DrawLifeFlaskLower(!draw.mainPanel);
	}
	if (draw.mana) {
		DrawManaFlaskLower(!draw.mainPanel);

		DrawSpell();
	}
	if (draw.controlButtons) {
...
void RedrawGameScene()
{
	...
	// A fade step repaints from scratch, so every component is drawn.
	DrawScene({
	    .mainPanel = true,
	    .health = true,
	    .mana = true,
...
void DrawAndBlit()
{
	...
	PanelDrawFlags draw {
		.mainPanel = false,
		.health = IsRedrawComponent(PanelDrawComponent::Health),
		.mana = IsRedrawComponent(PanelDrawComponent::Mana),
		.controlButtons = IsRedrawComponent(PanelDrawComponent::ControlButtons),
		.belt = IsRedrawComponent(PanelDrawComponent::Belt),
		.chatInput = ChatFlag,
	};
	...
	MarkDirtyRegions(mainPanel, draw.health, draw.mana, draw.belt, draw.controlButtons, draw.chatInput);
