...
{
	// Hide lighting modes the active renderer does not support from the settings UI.
	GetOptions().Graphics.lightingMode.SetEntryFilter([](int mode) {
		return CurrentRenderer == nullptr || CurrentRenderer->SupportsLightingMode(static_cast<LightingMode>(mode));
	});
