	...
	for (size_t pageEntry = 0; pageEntry < SpellBookPageEntries; pageEntry++) {
		...
		if (IsValidSpell(sn) && (spl & GetSpellBitmask(sn)) != 0) {
			...
			case SpellType::Charges: {
				...
				PrintSBookStr(line1, FormatRuntime(ngettext("Staff ({:d} charge)", "Staff ({:d} charges)", charges), charges));
			} break;
			default: {
				const int mana = GetManaAmount(player, sn) >> 6;
				const int lvl = player.GetSpellLevel(sn);
				PrintSBookStr(line0, FormatRuntime(pgettext(/* TRANSLATORS: UI constraints, keep short please.*/ "spellbook", "Level {:d}"), lvl), UiFlags::AlignRight);
				if (const StringOrView text = GetSpellPowerText(sn, lvl); !text.empty()) {
					PrintSBookStr(line1, text, UiFlags::AlignRight);
				}
				PrintSBookStr(line1, FormatRuntime(pgettext(/* TRANSLATORS: UI constraints, keep short please.*/ "spellbook", "Mana: {:d}"), mana));
			} break;
			}
		}
