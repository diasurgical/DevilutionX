...
void DrawAutocompleteSuggestions(int screenWidth, const std::vector<LuaAutocompleteSuggestion> &suggestions, Point position)
{
	...
	GetRenderer().FillRect({ position, { outerWidth, height } }, PAL16_BLUE + 14);
	size_t i = 0;
	...
	for (const LuaAutocompleteSuggestion &suggestion : suggestions) {
		...
		GetRenderer().SetClipRegion({ textPosition, { maxInnerWidth, textHeight } });
		DrawString(
    ...
    Rectangle rect, std::string_view originalInputText, std::string_view wrappedInputText)
{
	...
	GetRenderer().SetClipRegion({ { absX, absY }, rect.size });
...
void DrawConsoleLines(const Rectangle &region)
{
	...
	GetRenderer().SetClipRegion(region);
...
void DrawConsole()
{
	...
	GetRenderer().DrawBlendedRect(bgRect);
	...
	GetRenderer().FillRect({ lineFrom, { InputRect.size.width, 1 } }, BorderColor);
	DrawInputText(
	    screenSize.width,
	    Rectangle(
